...
- No new release/tag is part of this task unless the user explicitly asks after the feature is complete.
...
3. Long-task recovery is mandatory: when the user says `ادامه بده`, re-read this file, re-read the `Last Work File`, verify branch HEAD/CI, and continue from `NEXT EXACT TASK`.
...
- No unnecessary dependencies, broad refactors, lockfile/config changes, or release/version changes.
- Dashboard committed build parity remains a required gate.

## Current Status

TASK STARTED. BRANCH CREATED FROM CURRENT MAIN. NO PRODUCT EDITS YET.
...
5. **Final verification**
   ...
   - Self-review full diff for permission leaks, N+1 queries, duplicate Users UI, unintended Plan/Access Group/accounting/release changes.
...
Task initialization and recovery checkpoint before the autofill audit.
