...
- The deployment target is now a **fresh installation** and the user explicitly accepts loss of prior-version database data (confirmed 2026-09-19). Historical row migration is therefore not a Product-first acceptance requirement. Keep legacy tables only while current runtime paths still reference them; remove them in a forward, tested schema-cleanup stage after those paths have moved to Product. The V1 Plan-commercial / Access Group-network rules below apply to those remaining legacy paths only; never silently broaden permissions.
