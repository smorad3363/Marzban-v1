...
const Kpi: FC<{ label: string; value: string; detail: string; icon: ReactElement; fullMobile?: boolean }> = ({ label, value, detail, icon, fullMobile = false }) => (
  <Card gridColumn={fullMobile ? { base: "span 2", sm: "auto" } : undefined} p={3.5} minH="118px" bg="var(--panel-surface)" color="inherit" borderWidth="1px" borderColor="var(--panel-border)" borderRadius="14px">
    <HStack justify="space-between" align="start" gap={3}>
      <Box minW={0}>
        <Text color="var(--panel-text-muted)" _dark={{ color: "gray.400" }} fontSize="xs" fontWeight="700">{label}</Text>
    ...
    <Text mt={2} color="var(--panel-text-muted)" _dark={{ color: "gray.400" }} fontSize="xs" lineHeight="1.6">{detail}</Text>
  </Card>
);
...
const ResourceMeter: FC<{ label: string; value: string; detail: string; percent?: number; icon: ReactElement }> = ({ label, value, detail, percent, icon }) => (
  <Box p={3} minW={0} bg="var(--panel-nested)" borderWidth="1px" borderColor="var(--panel-border)" borderRadius="12px">
    <HStack justify="space-between" align="start" gap={3}>
      <Box minW={0}><Text color="var(--panel-text-muted)" _dark={{ color: "gray.400" }} fontSize="xs">{label}</Text><Text mt={1} fontSize="lg" fontWeight="800" dir="ltr">{value}</Text></Box>
    ...
    <Text mt={2} color="var(--panel-text-muted)" fontSize="xs">{detail}</Text>
  </Box>
);
    ...
    <Stack spacing={3} mb={5} aria-live="polite">
            ...
            <Stack spacing={2.5}>
              <HStack justify="space-between" align="start" gap={3}>
                <Box minW={0}>
                  ...
                  <Text mt={2} color="var(--panel-text-muted)" _dark={{ color: "gray.400" }} fontSize="xs">{accountData.role === "OWNER" ? "دسترسی مالک" : "اعتبار قابل استفاده"}</Text>
              ...
              {accountData.role === "OWNER" ? (
                <Text color="var(--panel-text-muted)" _dark={{ color: "gray.400" }} fontSize="xs">تمام محدودیت‌های تجاری حساب برای مالک غیرفعال است.</Text>
              ) : (
                <Text color="var(--panel-text-muted)" _dark={{ color: "gray.400" }} fontSize="xs">{accountData.billing_mode === "USED_TRAFFIC" ? `قیمت خرید هر گیگ: ${faNumber(accountData.used_traffic_price_per_gib_toman || 0)} تومان` : "خرید و تمدید از قیمت پلن کسر می‌شود."}</Text>
              )}
            </Stack>
          )}
        </Card>
      ...
      <SimpleGrid columns={{ base: 1, xl: 5 }} gap={3}>
        <Card p={3.5} minW={0} gridColumn={{ xl: "span 3" }} bg="var(--panel-surface)" color="inherit" borderWidth="1px" borderColor="var(--panel-border)" borderRadius="14px">
          <Box><Text as="h2" fontWeight="800">ترکیب وضعیت کاربران</Text><Text mt={1} color="var(--panel-text-muted)" _dark={{ color: "gray.400" }} fontSize="xs">هر کاربر دقیقاً در یکی از وضعیت‌های زیر شمرده می‌شود.</Text></Box>
          {statusTotal === 0 ? <Text py={12} textAlign="center" color="var(--panel-text-muted)">هنوز کاربری ثبت نشده است.</Text> : (
              ...
              <Stack spacing={1.5}>
                {statusItems.map((item) => (
                  <HStack key={item.label} minH="34px" px={2.5} py={1.5} justify="space-between" bg="var(--panel-nested)" borderWidth="1px" borderColor="var(--panel-border)" borderRadius="9px">
                    <HStack><Box boxSize="8px" borderRadius="full" bg={item.color} /><Text color="var(--panel-text-body)" fontSize="xs">{item.label}</Text></HStack>
        ...
        <Card p={3.5} minW={0} gridColumn={{ xl: "span 2" }} bg="var(--panel-surface)" color="inherit" borderWidth="1px" borderColor="var(--panel-border)" borderRadius="14px">
          <HStack justify="space-between" align="start" gap={3}><Box><Text as="h2" fontWeight="800">آخرین فعالیت‌ها</Text><Text mt={1} color="var(--panel-text-muted)" _dark={{ color: "gray.400" }} fontSize="xs">رخدادهای واقعی در محدوده مدیریتی شما</Text></Box><ClockIcon width={19} color="var(--panel-accent)" aria-hidden="true" /></HStack>
          {activity.isLoading ? <Stack mt={3}>{Array.from({ length: 4 }).map((_, index) => <Skeleton key={index} h="42px" borderRadius="9px" />)}</Stack> : activity.isError ? (
            ...
            <Text mt={5} color="var(--panel-text-muted)" fontSize="sm">هنوز فعالیتی ثبت نشده است.</Text>
          ) : (
            <Stack mt={3} spacing={1.5}>
              {activity.data.map((item) => (
                <HStack key={item.id} minH="42px" px={2.5} py={1.5} justify="space-between" align="center" gap={3} bg="var(--panel-nested)" borderWidth="1px" borderColor="var(--panel-border)" borderRadius="9px">
                  ...
                  <Text flexShrink={0} color="var(--panel-text-muted)" fontSize="10px" dir="ltr">{activityTime(item.created_at)}</Text>
                </HStack>
              ))}
            </Stack>
          )}
      ...
      {trafficModes.length > 1 && (
        <Card p={3.5} bg="var(--panel-surface)" color="inherit" borderWidth="1px" borderColor="var(--panel-border)" borderRadius="14px" minW={0}>
          <HStack justify="space-between" align="start" gap={3} flexWrap="wrap"><Box><Text as="h2" fontWeight="800">ترافیک به تفکیک نوع اعتبار</Text><Text mt={1} color="var(--panel-text-muted)" _dark={{ color: "gray.400" }} fontSize="xs">فقط مدل‌هایی نمایش داده می‌شوند که مصرف یا حجم تعریف‌شده دارند.</Text></Box><Badge colorScheme="yellow">GB</Badge></HStack>
      ...
      <HStack color="var(--panel-text-muted)" fontSize="10px" justify="end"><ClockIcon width={13} aria-hidden="true" /><Text>آخرین محاسبه: {new Date(data.generated_at).toLocaleTimeString("fa-IR", { hour: "2-digit", minute: "2-digit" })}</Text></HStack>
      ...
      {accountData?.account_status === "ACTIVE" && <Box position="fixed" left={{ base: 4, md: 6 }} right="auto" bottom={{ base: 4, md: 6 }} zIndex="popover">
        ...
        <IconButton aria-label={quickOpen ? "بستن دسترسی سریع" : "بازکردن دسترسی سریع"} aria-expanded={quickOpen} icon={<BoltIcon width={24} />} onClick={() => setQuickOpen((value) => !value)} boxSize="54px" borderRadius="full" colorScheme="primary" color="var(--panel-accent-contrast)" boxShadow="0 12px 30px rgba(0,0,0,.45)" />
      </Box>}
