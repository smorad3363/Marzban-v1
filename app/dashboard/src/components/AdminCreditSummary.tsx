...
export const AdminCreditSummary: FC = () => {
  ...
  return (
    <Stack spacing={3} mb={5}>
      ...
      <Card bg="var(--panel-surface)" color="var(--panel-text)" borderWidth="1px" borderColor="var(--panel-border)" borderRadius="16px" boxShadow="panel" overflow="hidden">
        <HStack px={{ base: 4, md: 5 }} py={3} spacing={2} flexWrap="wrap" borderBottomWidth="1px" borderColor="var(--panel-border)">
        ...
        <SimpleGrid columns={{ base: 1, md: 3 }}>
          <Box p={{ base: 4, md: 5 }} borderInlineEndWidth={{ base: 0, md: "1px" }} borderColor="var(--panel-border)">
            ...
            <Text color="var(--panel-text-muted)" fontSize="xs" mt={2}>{account.billing_mode === "USED_TRAFFIC" ? `قیمت خرید هر گیگ: ${(account.used_traffic_price_per_gib_toman || 0).toLocaleString("fa-IR")} تومان` : "هر ساخت یا تمدید، قیمت همان پلن را از کیف پول کم می‌کند."}</Text>
          </Box>
          <Box p={{ base: 4, md: 5 }} borderTopWidth={{ base: "1px", md: 0 }} borderInlineEndWidth={{ base: 0, md: "1px" }} borderColor="var(--panel-border)">
            <Text color="var(--panel-text-muted)" fontSize="xs" fontWeight="650">کاربران زیرمجموعه</Text>
            <Text color="white" fontSize="2xl" fontWeight="800" mt={2}>{account.subtree_users}</Text>
            <Text color="var(--panel-text-muted)" fontSize="xs" mt={2}>{account.own_users} مستقیم · {account.subtree_users} با زیرمجموعه‌ها</Text>
          </Box>
          <Box p={{ base: 4, md: 5 }} borderTopWidth={{ base: "1px", md: 0 }} borderColor="var(--panel-border)">
            <Text color="var(--panel-text-muted)" fontSize="xs" fontWeight="650">اعتبار تمدید باقی‌مانده</Text>
            ...
            <Text color="var(--panel-text-muted)" fontSize="xs" mt={2}>{account.renewal_enabled ? "تمدید فعال است" : "تمدید بسته است"}</Text>
          </Box>
        </SimpleGrid>
      </Card>
    </Stack>
